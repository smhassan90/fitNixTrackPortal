...
import { EXPENSE_TEAM_PERMISSION_DEFS } from '@/lib/expensePermissions';
...
export default function TeamPage() {
  ...
  const loadCatalog = useCallback(async () => {
    try {
      ...
      for (const p of [...POS_TEAM_PERMISSION_DEFS, ...EMPLOYEE_TEAM_PERMISSION_DEFS, ...EXPENSE_TEAM_PERMISSION_DEFS]) {
        if (!keys.has(p.key)) merged.push({ ...p });
      }
      setCatalog(merged);
      setAlwaysAvailable(data.alwaysAvailable);
    } catch {
      setCatalog([...POS_TEAM_PERMISSION_DEFS, ...EMPLOYEE_TEAM_PERMISSION_DEFS, ...EXPENSE_TEAM_PERMISSION_DEFS]);
      setAlwaysAvailable([
        {
